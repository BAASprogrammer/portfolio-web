...
export const Introduction = () => {
  ...
  return (
    <div className="relative">
      ...
      <button
        className='md:hidden fixed top-6 right-6 z-50 p-3 rounded-full bg-slate-900/90 border border-white/10 text-white shadow-2xl backdrop-blur-xl'
      ...
      <aside className={`
        ...
        <div className="px-6 pt-24 pb-8 md:pt-14 text-default flex flex-col min-h-full border-r border-white/5">
          ...
          <div className="mb-10 md:mb-12">
            ...
            <h1 className="text-2xl font-black tracking-tighter text-white">Bárbara Arias</h1>
          </div>
          ...
          <nav className="flex-1 space-y-1">
            {[
              ...
              { id: 'skills', label: 'Habilidades', icon: <FaTerminal size={14} /> },
              { id: 'experience', label: 'Experiencia', icon: <FaBriefcase size={14} /> },
              { id: 'projects', label: 'Proyectos', icon: <FaCode size={14} /> },
              { id: 'contact', label: 'Contacto', icon: <FaEnvelope size={14} /> }
            ].map((item) => (
              <button
                key={item.id}
                onClick={() => { scrollTo(item.id); setMenuOpen(false); }}
                className={`${menuClass(item.id)} text-sm uppercase tracking-widest relative overflow-hidden w-full cursor-pointer`}
              >
                <span className="opacity-70 group-hover/item:opacity-100 transition-opacity">{item.icon}</span>
                <span className="transition-transform duration-300 group-hover/item:translate-x-1">{item.label}</span>
                {activeSection === item.id && (
                  <div className="absolute left-0 top-[20%] bottom-[20%] w-[3px] bg-pink-500 rounded-r-full shadow-lg shadow-pink-500/50" />
                )}
              </button>
            ))}
          </nav>
          ...
          <div className="mt-8 pt-8 border-t border-white/5 space-y-6">
            ...
            <div className="px-4 py-3 bg-white/5 rounded-xl border border-white/5">
              <p className="text-[11px] text-gray-400 font-medium leading-relaxed text-center sm:text-left">
